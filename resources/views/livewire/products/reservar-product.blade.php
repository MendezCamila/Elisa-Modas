        ...
        <div class="card">
            <div class="grid md:grid-cols-2 gap-6">
                ...
                <div class="col-span-1">
                    <figure>
                        <img src="{{ $this->currentImage }}" class="aspect-[1/1] w-full object-cover object-center" alt="">
                    </figure>
                </div>
                ...
                <div class="col-span-1">
                    <div>
                        ...
                        <div x-data="{ qty: @entangle('qty'), stock: @entangle('currentStock') }" class="flex space-x-6 items-center mb-6">
                            <button @click="qty > 1 ? qty-- : qty" class="btn btn-gray" x-bind:disabled="qty == 1">-</button>
                                        ...
                                        @foreach ($option->pivot->features as $feature)
                                            <li>
                                                @switch($option->type)
                                                    @case(1)
                                                        {{-- es de tipo texto --}}
                                                        <button class="w-20 h-8 font-semibold uppercase text-sm rounded-lg border-gray-200 text-gray-700 {{ $selectedFeatures[$option->id] == $feature['id'] ? ' bg-pink-500 text-white' : ' bg-gray-200' }}" wire:click="$set('selectedFeatures.{{ $option->id }}', {{ $feature['id'] }})">
                                                            {{ $feature['value'] }}
                                                        </button>
                                                    @break
                                                    @case(2)
                                                        ...
                                                        <div class="p-0.5 border-2 rounded-lg flex items-center -mt-1.5 {{ $selectedFeatures[$option->id] == $feature['id'] ? ' border-pink-500' : 'border-transparent' }}">
                                                            <button class="w-20 h-8 rounded-lg border border-gray-200" style="background-color: {{ $feature['value'] }}" wire:click="$set('selectedFeatures.{{ $option->id }}', {{ $feature['id'] }})"></button>
                                                        </div>
                                                    @break
                                                    @default
                                                @endswitch
                        ...
                        <button class="btn btn-pink w-full mb-6" wire:click="reservar" wire:loading.attr="disabled">
                            Reservar
                        </button>
                    </div>
                </div>
