<div>
            ...
            <div class="space-y-6">
                ...
                @foreach ($options as $option)
                    <div class="p-6 rounded-lg border border-gray-200 relative"
                        ...
                        <div class="flex flex-wrap mb-4">
                            ...
                            @foreach ($option->features as $feature )
                                ...
                                @switch($option->type)
                                    @case(1)
                                        {{-- texto --}}
                                        <span class="bg-gray-100 text-gray-800 text-xs font-medium me-2 pl-2.5 pr-1.5 py-0.5 rounded dark:bg-gray-700 dark:text-gray-400 border border-gray-500 mr-4">
                                            {{ $feature->description }}
                                    ...
                                    @case(2)
                                        {{-- color --}}
                                        <div class="relative">
                                            <span class="inline-block h-6 w-6 shadow-lg rounded-full border-2 border-gray-400 mr-4" style="background-color:{{ $feature->value }}"></span>

                                            <button class="absolute z-10 left-3 -top-2 rounded-full bg-red-600 hover:bg-red-800 h-4 w-4 flex justify-center items-center"
                                                onclick="ConfirmDelete({{ $feature->id }},'feature')">
                        ...
                        {{-- Incluir el componente addNewFeature --}}
                        {{-- Este componente va a agregar nuevos features a las opciones
                        pero va a agregarle a una opcion en particular --}}
                        <div>
    ...
    <x-dialog-modal wire:model="newOption.openModal">
        {{-- cualquier cosa que definamos en el slot va
        a aparecer donde corresponde title --}}
        <x-slot name="title">
            Crear nueva opcion
        </x-slot>
